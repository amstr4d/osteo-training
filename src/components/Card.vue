  ...
  <div class="rounded overflow-hidden shadow mb-4 bg-white dark:bg-gray-700">
    ...
    <div class="px-6 pt-4 pb-2">
      <div class="flex justify-between">
        <button @click="$refs.deleteConfirm.openModal()" class="text-red-500 focus:outline-none">
    ...
    <modal ref="deleteConfirm">
      <template v-slot:body>
        <p class="text-xl text-center">Veuillez confirmer la suppression</p>
      </template>
      <template v-slot:footer>
          <button class="hover:text-teal-600 focus:outline-none" @click="$refs.deleteConfirm.closeModal()">Annuler</button>
          <button class="btn" @click="deleteSentence(sentence.id)">Supprimer</button>
      </template>
    </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteSentence(id) {
      if (id) {
        this.$refs.deleteConfirm.closeModal();
